    ...
    try {
      const rawPayload =
        typeof data.rawData === "string"
          ? JSON.parse(data.rawData as string)
          : data.data
      const status =
        (rawPayload?.status as string | undefined) ||
        (rawPayload?.data?.status as string | undefined) ||
        (rawPayload?.invoice?.status as string | undefined) ||
        ""
      const invoiceId =
        (rawPayload?.invoiceId as string | undefined) ||
        (rawPayload?.data?.invoiceId as string | undefined) ||
        (rawPayload?.invoice?.id as string | undefined) ||
        (rawPayload?.id as string | undefined)
      const amountValue =
        rawPayload?.amount || rawPayload?.data?.amount || rawPayload?.invoice?.amount || 0
      ...
      if (["settled", "paid", "complete", "completed"].includes(normalizedStatus)) {
        return {
          action: "captured",
          data: {
      ...
      if (["invalid", "expired", "failed"].includes(normalizedStatus)) {
        return {
          action: "failed",
          data: {
            session_id: invoiceId,
